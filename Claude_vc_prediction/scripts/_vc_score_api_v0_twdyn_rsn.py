"""
Claude API Visual Complexity (VC) Scoring — V0+TW-rsn (reasoning-field)
========================================================================
V0+TW (dynamic-W) with the output JSON modified to add a leading
"reasoning" field that the model must populate *before* vc_score. The
autoregressive decoder therefore conditions the score on a verbalised
analysis of the image. Unlike V0+TW-det, this is a single free-form
field rather than seven numeric per-dimension fields, so it should not
compress prediction spread the way -det does.

Usage:
    python _vc_score_api_v0_twdyn_rsn.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twdyn_rsn --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_twdyn_rsn'

# ── V0+TW-rsn System Prompt — topics + dynamic weights + reasoning field ──
...
## Output Format
Return ONLY valid JSON (no markdown fences, no explanation outside JSON). The fields MUST appear in this exact order:
{
  "reasoning": "<3-4 sentences of private analysis: identify which of the 7 topics are most salient for this specific visualization, cite the visual evidence, and articulate how the high-weight topics combine. Write this BEFORE deciding vc_score.>",
...
def append_explanations_row(expl_csv: Path, filename: str, result: dict):
    ...
    with open(expl_csv, 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(['filename', 'reasoning', 'explanation'])
        writer.writerow([filename, result.get('reasoning', ''), result.get('explanation', '')])
...
def main():
    parser = argparse.ArgumentParser(description='V0+TW-dyn VC Scoring via Claude API')
    ...
    print(f'=== V0+TW-rsn (V0+TW-dyn + leading reasoning JSON field) ===')
    print(f'Model:          {MODEL}')
    print(f'Prompt:         V0 + 7 topic descriptions + dynamic weight instruction (no calibration, no anchors)')
