"""
Vanilla V0+T VC Scoring — No Role Framing
==========================================
Identical to _vc_score_api_v0_topic.py except the expert-role sentence
"You are a visual complexity (VC) scoring expert for data visualizations."
is removed from the system prompt.

Usage:
    python _vc_score_api_vanilla_t.py --input-csv ../Claude_vc_prediction/gt_all_66.csv --outdir ../results/vc_api_63_vanilla_t --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_63_vanilla_t'

# ── Vanilla V0+T — no role framing ───────────────────────────────────────────
...
async def _run_concurrent(api_key, to_process, scores_csv, expl_csv, concurrency):
    ...
    tasks   = [_score_one_async(aclient, sem, i, total, r['imageName'], r['imageURL'], scores_csv, expl_csv)
               for i, (total, r) in enumerate([(len(to_process), r) for r in to_process], 1)]
    # rebuild properly
...
def main():
    parser = argparse.ArgumentParser(description='Vanilla V0+T VC Scoring (no role framing)')
    ...
    print('=== Vanilla V0+T (no role framing) ===')
    print(f'Model:        {MODEL}')
