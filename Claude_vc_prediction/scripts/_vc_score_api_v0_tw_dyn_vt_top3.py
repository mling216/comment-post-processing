"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Weighted-Dynamic+VisTypeDefs (Top-3 variant)
==================================================================================================
V0+TW(dyn)+VT: V0+TW-dyn prompt extended with the 9 vis-type definitions so the
LLM can identify the visualization type and apply type-appropriate complexity
reasoning.  Uses dynamic (model-decided) topic weighting rather than fixed
H/M/L weights.  Includes the top-3 topic-selection task.

Usage:
    python _vc_score_api_v0_tw_dyn_vt_top3.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_topicsel_v0_tw_dyn_vt --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_topicsel_v0_tw_dyn_vt'

DIM_KEYS = ['data_density', 'visual_encoding', 'text_annotation',
            'domain_schema', 'color_symbol', 'aesthetic_order', 'cognitive_load']

# ── V0+Topic+Weighted-Dynamic+VisTypeDefs System Prompt ──────────────────────
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Weighted-Dynamic+VisTypeDefs VC Scoring via Claude API')
