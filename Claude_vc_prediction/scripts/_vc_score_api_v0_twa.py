"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Weighted+Anchors (V0+TWA)
================================================================================
V0 baseline + 7 topic descriptions + qualitative weight guidance + 3 anchor
images (vc_score only, no per-dimension values). No calibration guidance.

This is the "T+W+A" triple: adds few-shot anchors on top of the best
double (V0+TW) to test whether concrete exemplars combine with the
weighted topic instruction.

Usage:
    python _vc_score_api_v0_twa.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twa --concurrency 5 --model claude-opus-4-6
    python _vc_score_api_v0_twa.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twa_r2 --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_twa'
...
# ── V0+TWA System Prompt ─ definition + 7 topics + weights (no calibration) ─
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Weighted+Anchors VC Scoring via Claude API')
    ...
    print(f'=== V0+Topic+Weighted+Anchors (topics + weights + 3 anchors) ===')
    print(f'Model:          {MODEL}')
    ...
    if concurrency == 1:
        ...
        ok, failed = asyncio.run(_run_concurrent(
            api_key, to_process, anchor_msgs, scores_csv, expl_csv, concurrency))

    print(f'\nDone: {ok}/{len(to_process)} succeeded.')
    if failed:
