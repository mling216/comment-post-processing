"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Weighted-Dynamic+Anchors (V0+TWdynA)
==========================================================================================
V0+TWA variant where the fixed High/Medium/Low topic weights are replaced
with a dynamic self-assigned weighting instruction. The model observes the
image and decides which topics matter most, then applies 3 few-shot anchor
images (vc_score only) for calibration.

Usage:
    python _vc_score_api_v0_twdyna.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twdyna --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_twdyna'
...
# ── V0+TWdynA System Prompt ─ definition + 7 topics + dynamic weights (no calibration) ─
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Weighted-Dynamic+Anchors VC Scoring via Claude API')
    ...
    print(f'=== V0+Topic+Weighted-Dynamic+Anchors (topics + self-assigned weights + 3 anchors) ===')
    print(f'Model:          {MODEL}')
