"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Calibration+Anchors
=========================================================================
Full ablation combining all three additions on top of the V0 baseline:
  - 7 topic descriptions (from V0+Topic)
  - Calibration guidance (from V0+Calibration)
  - 3 anchor images with vc_score only (from V0+Anchors)

Usage:
    python _vc_score_api_v0_topic_calib_anchor.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_tca --concurrency 5
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_tca'
...
# ── System Prompt — V0 + Topics + Calibration ─────────────────────────────
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Calibration+Anchors VC Scoring via Claude API')
    ...
    print(f'=== V0+Topic+Calibration+Anchors ===')
    print(f'Model:          {MODEL}')
    print(f'Prompt:         V0 definition + 7 topics + calibration + 3 anchors')
