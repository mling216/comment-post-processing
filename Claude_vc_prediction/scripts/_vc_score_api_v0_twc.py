"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Weighted+Calibration (V0+TWC)
===================================================================================
V0 baseline + 7 topic descriptions + qualitative weight guidance + calibration
ranges. No anchor exemplars. Output schema remains V0-style (single holistic
vc_score + short explanation).

This is the "T+W+C" triple: adds calibration guidance on top of the best
double (V0+TW) to test whether scale anchoring pushes CCC further.

Usage:
    python _vc_score_api_v0_twc.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twc --concurrency 5 --model claude-opus-4-6
    python _vc_score_api_v0_twc.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twc_r2 --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_twc'

# ── V0+TWC System Prompt ─ definition + 7 topics + weights + calibration ─
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Weighted+Calibration VC Scoring via Claude API')
    ...
    print(f'=== V0+Topic+Weighted+Calibration (topics + weights + calibration) ===')
    print(f'Model:          {MODEL}')
