"""
Claude API Visual Complexity (VC) Scoring — V0+TWdynCA (V0+TWCA with dynamic weights)
=======================================================================================
Same as V0+TWCA (topics + calibration guidance + 3 anchors) but the fixed
High/Medium/Low topic weights are replaced with a dynamic self-assigned
weighting instruction — the model observes the image and decides which
topics matter most for that specific visualization.

Usage:
    python _vc_score_api_v0_twdynca.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twdynca --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_twdynca'
...
# ── System Prompt — V0 + Topics + Calibration + Dynamic Weighted Scoring ──
...
Return ONLY valid JSON (no markdown fences, no explanation outside JSON):
{
  ...
  "explanation": "<2-3 sentence justification naming the dimensions that most influenced the score>"
}"""
...
def main():
    parser = argparse.ArgumentParser(description='V0+TWdynCA VC Scoring via Claude API')
    ...
    print(f'=== V0+TWdynCA (topics + self-assigned weights + calibration + 3 anchors) ===')
    print(f'Model:          {MODEL}')
