"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Calibration
=================================================================
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Calibration VC Scoring via Claude API')
    ...
    print(f'=== V0+Topic+Calibration (topics + calibration, no anchors) ===')
    print(f'Model:          {MODEL}')
    ...
    if concurrency == 1:
        ...
        ok, failed = asyncio.run(_run_concurrent(
            api_key, to_process, scores_csv, expl_csv, concurrency))
