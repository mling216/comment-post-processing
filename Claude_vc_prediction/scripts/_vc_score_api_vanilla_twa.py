"""
Vanilla V0+TWA VC Scoring — No Role Framing
============================================
Combines V0+TW-dyn (7 topics + dynamic self-weighting) with V0+A (3 anchor
few-shot exemplars). The expert-role sentence is omitted (vanilla variant).

Anchors (same 3 few-shot images, excluded from metrics):
  Low  (0.22): VisC.503.6.png
  Mid  (0.54): InfoVisJ.619.17.png
  High (0.95): InfoVisJ.1149.6(1).png

Usage:
    python _vc_score_api_vanilla_twa.py --input-csv ../Claude_vc_prediction/gt_all_66.csv --outdir ../results/vc_api_63_vanilla_twa --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_63_vanilla_twa'
...
# ── Vanilla V0+TWA — topics + dynamic weighting + anchors, no role ────────
SYSTEM_PROMPT = """**Visual Complexity** measures the amount of detail, intricacy, and perceptual/cognitive demand of a visualization image. It is scored on a 0–1 scale where 0 = minimal complexity and 1 = maximum complexity.
...
Before arriving at the final vc_score, examine which of the 7 topics are most salient for THIS specific visualization. Assign your own relative importance to each topic based on what you observe in the image — no fixed priority is imposed. Let the image's characteristics guide which topics matter most for this particular case.
...
def append_scores_row(scores_csv: Path, filename: str, result: dict):
    ...
    with open(scores_csv, 'a', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        if write_header:
            w.writerow(['filename', 'vc_score'])
        w.writerow([filename, result.get('vc_score', '')])
...
def append_explanations_row(expl_csv: Path, filename: str, result: dict):
    ...
    with open(expl_csv, 'a', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        if write_header:
            w.writerow(['filename', 'explanation'])
        w.writerow([filename, result.get('explanation', '')])
...
def build_anchor_messages() -> list:
    ...
    for anchor in ANCHORS:
        ...
        messages.append({"role": "user", "content": [
            {"type": "image", "source": {"type": "base64", "media_type": get_media_type(img_name), "data": b64}},
        ...
        messages.append({"role": "assistant", "content": json.dumps(response_obj)})
    # cache_control on last anchor message
    if messages:
        last = messages[-1]
...
async def _score_one_async(aclient, sem, idx, total, img_name, img_url, anchor_msgs, scores_csv, expl_csv):
    async with sem:
        b64 = _load_image(img_name, url=img_url)
        if b64 is None:
            print(f'[{idx}/{total}] {img_name} ... SKIP')
            return (img_name, None)
        messages = list(anchor_msgs) + [{"role": "user", "content": [
            {"type": "image", "source": {"type": "base64", "media_type": get_media_type(img_name), "data": b64}},
            {"type": "text",  "text": "Score the visual complexity of this visualization image."}
        ]}]
        try:
...
async def _run_concurrent(api_key, to_process, anchor_msgs, scores_csv, expl_csv, concurrency):
    ...
    tasks   = [_score_one_async(aclient, sem, i, total, r['imageName'], r['imageURL'], anchor_msgs, scores_csv, expl_csv)
               for i, r in enumerate(to_process, 1)]
    results = await asyncio.gather(*tasks)
    ok      = sum(1 for _, vc in results if vc is not None)
    failed  = [n for n, vc in results if vc is None]
    return ok, failed

def _run_sequential(client, to_process, anchor_msgs, scores_csv, expl_csv):
    ok, failed = 0, []
    for i, row in enumerate(to_process, 1):
        img_name, img_url = row['imageName'], row['imageURL']
        print(f'[{i}/{len(to_process)}] {img_name} ... ', end='', flush=True)
        try:
            b64 = _load_image(img_name, url=img_url)
            if b64 is None:
                print('SKIP'); failed.append(img_name); continue
            messages = list(anchor_msgs) + [{"role": "user", "content": [
                {"type": "image", "source": {"type": "base64", "media_type": get_media_type(img_name), "data": b64}},
                {"type": "text",  "text": "Score the visual complexity of this visualization image."}
            ]}]
            ...
            print(f'JSON PARSE ERROR: {e}'); failed.append(img_name)
        except anthropic.RateLimitError:
            print('RATE LIMITED — waiting 60s ...'); time.sleep(60); failed.append(img_name)
        except Exception as e:
            print(f'ERROR: {e}'); failed.append(img_name)
        if i < len(to_process):
...
def main():
    parser = argparse.ArgumentParser(description='Vanilla V0+TWA VC Scoring (topics + dynamic weighting + anchors, no role framing)')
    ...
    print('=== Vanilla V0+TWA (topics + dynamic weighting + anchors, no role framing) ===')
    print(f'Model:        {MODEL}')
    print(f'Anchors:      low=0.22, mid=0.54, high=0.95')
    print(f'Output dir:   {outdir.resolve()}')
    print(f'Total images: {len(all_images)}')
    print(f'Already done: {len(all_images) - len(to_process)}')
    ...
    if concurrency == 1:
        ...
        ok, failed = asyncio.run(_run_concurrent(api_key, to_process, anchor_msgs, scores_csv, expl_csv, concurrency))
