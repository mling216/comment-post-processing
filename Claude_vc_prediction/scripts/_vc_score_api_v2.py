"""
Claude API Visual Complexity (VC) Scoring — V2 Definition + Anchors
===================================================================
V0 prompt (VC definition only, no dimension descriptions) plus 3 few-shot
anchor images with only their vc_score. No dimension scores, no explanations,
no topics, no calibration guidance.

Usage:
    python _vc_score_api_v2.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v2 --concurrency 5
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v2'
...
# ── V2 System Prompt — Definition Only (same as V0) ───────────────────────
...
def build_anchor_messages() -> list:
    ...
    for anchor in ANCHORS:
        ...
        # Assistant turn: just vc_score + short explanation
        response_obj = {
...
def main():
    parser = argparse.ArgumentParser(description='V2 Definition + Anchors VC Scoring via Claude API')
    ...
    print(f'=== V2 Definition + 3 Anchors ===')
    print(f'Model:          {MODEL}')
    print(f'Prompt:         VC definition + 3 anchors (vc_score only)')
