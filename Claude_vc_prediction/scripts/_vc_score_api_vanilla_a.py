"""
Vanilla V0+A VC Scoring — No Role Framing
==========================================
Identical to _vc_score_api_v0_anchors.py except the expert-role sentence
"You are a visual complexity (VC) scoring expert for data visualizations."
is removed from the system prompt.

Anchors (same 3 few-shot images):
  Low  (0.22): VisC.503.6.png
  Mid  (0.54): InfoVisJ.619.17.png
  High (0.95): InfoVisJ.1149.6(1).png

Usage:
    python _vc_score_api_vanilla_a.py --input-csv ../Claude_vc_prediction/gt_all_66.csv --outdir ../results/vc_api_63_vanilla_a --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_63_vanilla_a'
...
def main():
    parser = argparse.ArgumentParser(description='Vanilla V0+A VC Scoring (no role framing)')
    ...
    print('=== Vanilla V0+A (no role framing) ===')
    print(f'Model:        {MODEL}')
