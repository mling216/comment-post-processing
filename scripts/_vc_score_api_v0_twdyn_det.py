"""
Claude API Visual Complexity (VC) Scoring — V0+TWdyn-det
=========================================================
V0 + Topics + dynamic Weights + per-dimension output schema, with NO
calibration ranges and NO anchor exemplars. Equivalent to V1 with both
the C (Calibration) and A (Anchors) blocks removed and weighting made
dynamic.

Tests whether dynamic weighting becomes useful once V1's calibration
and anchor supervision (which encode an implicit dimension hierarchy
that conflicts with self-assigned weights) are stripped away.

Usage:
    python _vc_score_api_v0_twdyn_det.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twdyn_det --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_twdyn_det'
...
def main():
    parser = argparse.ArgumentParser(description='V0+TWdyn-det VC Scoring via Claude API')
    ...
    print(f'=== V0+TWdyn-det (topics + DYNAMIC weights + per-dim output, NO calibration, NO anchors) ===')
    print(f'Model:          {MODEL}')
    ...
    print('Skipping anchor injection (V0+TWdyn-det carries no anchors).')
    anchor_messages = []
