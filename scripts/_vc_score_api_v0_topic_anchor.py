"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Anchors
=============================================================
V0 baseline + 7 topic descriptions + 3 anchor images. No calibration guidance.

Usage:
    python _vc_score_api_v0_topic_anchor.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_ta --concurrency 5
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_ta'
...
# ── System Prompt — V0 + Topics + Anchors (no calibration) ────────────────
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Anchors VC Scoring via Claude API')
    ...
    print(f'=== V0+Topic+Anchors (topics + anchors, no calibration) ===')
    print(f'Model:          {MODEL}')
