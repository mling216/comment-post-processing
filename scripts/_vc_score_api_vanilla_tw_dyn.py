"""
Vanilla Zero-Shot V0+TW-dyn VC Scoring — No Role Framing
=========================================================
Identical to _vc_score_api_v0_tw_dyn.py except the expert-role sentence
"You are a visual complexity (VC) scoring expert for data visualizations."
is removed from the system prompt.

Usage:
    python _vc_score_api_vanilla_tw_dyn.py --input-csv ../Claude_vc_prediction/gt_all_66.csv --outdir ../results/vc_api_63_vanilla_tw_dyn --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_63_vanilla_tw_dyn'

# ── Vanilla V0+TW-dyn — no role framing ─────────────────────────────────────
...
async def _score_one_async(aclient, sem, idx, total, img_name, img_url, scores_csv, expl_csv):
    async with sem:
        ...
        try:
            response = await aclient.messages.create(
                ...
                temperature=0, system=[{"type": "text", "text": SYSTEM_PROMPT}]
            )
...
async def _run_concurrent(api_key, to_process, scores_csv, expl_csv, concurrency):
    ...
    tasks   = [_score_one_async(aclient, sem, i, total, r['imageName'], r['imageURL'], scores_csv, expl_csv)
...
def main():
    parser = argparse.ArgumentParser(description='Vanilla V0+TW-dyn VC Scoring (no role framing)')
    ...
    print('=== Vanilla V0+TW-dyn (no role framing) ===')
    print(f'Model:        {MODEL}')
    ...
    if concurrency == 1:
        ...
        ok, failed = asyncio.run(_run_concurrent(api_key, to_process, scores_csv, expl_csv, concurrency))
