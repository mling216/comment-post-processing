"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Weighted-Dynamic (Top-3 variant)
======================================================================================
Like V0+TW, but instead of fixed High/Medium/Low topic weights, the model is
instructed to decide its own relative weights by observing the image.
Returns `vc_score`, `explanation`, and `top3_topics` (same schema as V0+TW).

Usage:
    python _vc_score_api_v0_tw_dyn_top3.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_topicsel_v0_tw_dyn --concurrency 5 --model claude-sonnet-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_topicsel_v0_tw_dyn'
...
# ── V0+Topic+Weighted-Dynamic System Prompt ─────────────────────────────────
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Weighted-Dynamic VC Scoring via Claude API')
    ...
    print(f'=== V0+Topic+Weighted-Dynamic + Top-3 (definition + 7 topics + self-assigned weights + top-3 selection) ===')
    print(f'Model:          {MODEL}')
    print(f'Prompt:         V0 + 7 topic descriptions + dynamic self-assigned weight guidance (no calibration, no anchors)')
