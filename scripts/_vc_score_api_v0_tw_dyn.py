"""
Claude API Visual Complexity (VC) Scoring — V0+TW-dyn
======================================================
V0+TW with the fixed High/Medium/Low topic weights replaced by a dynamic
self-assigned weighting instruction. Output schema is V0-style (single
holistic vc_score + explanation). No top3 topics output.

Based on _vc_score_api_v0_topic_weighted.py — identical except for the
## Weighting of Topics section in the system prompt.

Usage:
    python _vc_score_api_v0_tw_dyn.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_tw_dyn --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_tw_dyn'

# ── V0+TW-dyn System Prompt — topics + dynamic self-assigned weighting ──────
...
def main():
    ...
    print(f'=== V0+TW-dyn (7 topics + dynamic self-assigned weights) ===')
    print(f'Model:          {MODEL}')
