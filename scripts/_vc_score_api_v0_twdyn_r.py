"""
Claude API Visual Complexity (VC) Scoring — V0+TW+R (reflection-instruction)
=============================================================================
V0+TW (dynamic-W) plus a single "think before answering" instruction
appended after Block W. Output schema is identical to V0+TW (holistic
vc_score + explanation); the only change is the extra reflection
paragraph in the system prompt.

Usage:
    python _vc_score_api_v0_twdyn_r.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_twdyn_r --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_twdyn_r'

# ── V0+TW+R System Prompt — topics + dynamic weights + reflection nudge ────
...
Before producing your JSON, examine the image carefully and silently work through how each high-weight topic manifests in this specific visualization. Take your time; accuracy matters more than brevity. Only after you have considered the evidence should you commit to a vc_score.
...
def main():
    ...
    print(f'=== V0+TW+R (V0+TW-dyn + reflection-before-answering paragraph) ===')
    print(f'Model:          {MODEL}')
