"""
Claude API Visual Complexity (VC) Scoring — V0+Calibration
===========================================================
V0 baseline + V3's calibration guidance (explicit score ranges + "push scores up"
instruction). No topics, no dimensions, no anchors. Output schema remains
...
def main():
    parser = argparse.ArgumentParser(description='V0+Calibration VC Scoring via Claude API')
    ...
    print(f'=== V0+Calibration (definition + V3 calibration guidance, no anchors) ===')
    print(f'Model:          {MODEL}')
    print(f'Prompt:         V0 + calibration guidance (no topics, no anchors)')
