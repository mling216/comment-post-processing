"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Weighted
=============================================================
V0 baseline + the 7 topic descriptions + qualitative weight guidance over
the 7 topics. No calibration ranges, no anchor exemplars. Output schema
remains V0-style (single holistic vc_score + short explanation).

This is the "T+W" ablation cell: it isolates whether reweighting the 7
topics (without calibration or anchors) moves the needle relative to
V0+Topic alone.

Usage:
    python _vc_score_api_v0_topic_weighted.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_tw --concurrency 5 --model claude-opus-4-6
    python _vc_score_api_v0_topic_weighted.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_tw_r2 --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_tw'
...
def main():
    ...
    print(f'=== V0+Topic+Weighted (definition + 7 topics + weight guidance) ===')
    print(f'Model:          {MODEL}')
