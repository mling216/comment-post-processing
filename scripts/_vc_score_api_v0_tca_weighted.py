"""
Claude API Visual Complexity (VC) Scoring — V0+TCA+Weighted
============================================================
Extends V0+TCA with qualitative per-dimension weight guidance (same as V3)
so the vc_score reflects a weighted holistic judgment rather than a simple average:

  High weight:   data_density, visual_encoding, domain_schema, cognitive_load
  Medium weight: color_symbol, aesthetic_order
  Low weight:    text_annotation
...
# ── System Prompt — V0 + Topics + Calibration + Weighted Scoring ──────────
...
def main():
    parser = argparse.ArgumentParser(description='V0+TCA+Weighted VC Scoring via Claude API')
    ...
    parser.add_argument('--thinking', action='store_true',
                        help='Enable adaptive thinking (omits temperature=0)')
    ...
    print(f'=== V0+TCA+Weighted ===')
    print(f'Model:          {MODEL}')
    print(f'Prompt:         V0 + 7 weighted dimensions + calibration + 3 anchors')
    print(f'Weights:        density/encoding/schema/cognitive=0.20, color/aesthetic=0.075, text=0.05')
