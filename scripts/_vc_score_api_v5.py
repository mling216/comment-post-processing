"""
Claude API Visual Complexity (VC) Scoring Script — V5
=====================================================
V5 = V3 + `user_topics` field in each anchor.

Each anchor's assistant JSON now includes the topics that real human observers
identified as driving their perception of visual complexity. The model is asked
to also predict user_topics for the target image (1–3 items from the 7 topic set).
All other aspects (3 anchors, system prompt, 7 dimension scores, calibration
guidance, caching) are identical to V3 — this is an ablation of the user_topics
feature only.

Usage:
    python _vc_score_api_v5.py                     # process all images from mapping CSV
    python _vc_score_api_v5.py --limit 5           # process first N images
    python _vc_score_api_v5.py --overwrite         # re-process already scored images
    python _vc_score_api_v5.py --source local      # use local image files
    python _vc_score_api_v5.py --source url        # fetch images from URLs (default)
    python _vc_score_api_v5.py --outdir results    # custom output directory
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_scores_v5'
...
# Closed vocabulary of user topics (from human study)
USER_TOPICS = [
...
ANCHORS = [
    {
        ...
        'user_topics': ['Schema', 'Immediacy / Cognitive Load']
    },
    {
        ...
        'user_topics': ['Data Density / Image Clutter', 'Immediacy / Cognitive Load']
    },
    {
        ...
        'user_topics': ['Data Density / Image Clutter', 'Semantics / Text Legibility', 'Aesthetics Uncertainty', 'Immediacy / Cognitive Load']
    },
]
...
## User Topics (new in V5)
Each anchor example includes a `user_topics` field: the topics that real human observers selected as the drivers of their complexity perception in a pair-comparison study. The 7 topics correspond conceptually to the 7 scoring dimensions above:
...
Use user_topics from the anchors to calibrate WHICH dimensions humans typically find most salient at each VC level. For the target image, also predict `user_topics`: select 1–3 topics (exactly as named above) that a typical human observer would cite when describing what makes this image simple or complex.
...
Return ONLY valid JSON (no markdown fences, no explanation outside JSON):
{
  ...
  "vc_score": <float 0-1>,
  "user_topics": [<1-3 topic names from the closed vocabulary above>],
...
def build_anchor_messages(source: str) -> list:
    ...
    for anchor in ANCHORS:
        ...
        response_obj['user_topics'] = anchor['user_topics']
        for dim in DIMENSIONS:
...
def append_scores_row(scores_csv: Path, filename: str, result: dict):
    ...
    with open(scores_csv, 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(['filename'] + DIMENSIONS + ['vc_score', 'user_topics'])
        topics = result.get('user_topics', [])
        if isinstance(topics, list):
            topics_str = '; '.join(topics)
        else:
            topics_str = str(topics) if topics else ''
        row = ([filename] + [result.get(d, '') for d in DIMENSIONS]
               + [result.get('vc_score', ''), topics_str])
        writer.writerow(row)
...
def main():
    parser = argparse.ArgumentParser(description='Score visual complexity of images via Claude API (V5: V3 + user_topics)')
