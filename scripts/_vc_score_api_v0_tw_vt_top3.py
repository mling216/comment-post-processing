"""
Claude API Visual Complexity (VC) Scoring — V0+Topic+Weighted+VisTypeDefs (Top-3 variant)
==========================================================================================
V0+TW+VT: V0+TW prompt extended with the 9 vis-type definitions so the LLM
can identify the visualization type and apply type-appropriate complexity
reasoning.  Also includes the top-3 topic-selection task and topic weights.

Usage:
    python _vc_score_api_v0_tw_vt_top3.py --input-csv ../Claude_vc_prediction/gt_all_66.csv --outdir ../results/vc_api_topicsel_v0_tw_vt --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_topicsel_v0_tw_vt'

DIM_KEYS = ['data_density', 'visual_encoding', 'text_annotation',
            'domain_schema', 'color_symbol', 'aesthetic_order', 'cognitive_load']

# ── V0+Topic+Weighted+VisTypeDefs System Prompt ──────────────────────────────
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic+Weighted+VisTypeDefs VC Scoring via Claude API')
