"""
Vanilla V0+C VC Scoring — No Role Framing
==========================================
Identical to _vc_score_api_v0_calibration.py except the expert-role sentence
"You are a visual complexity (VC) scoring expert for data visualizations."
is removed from the system prompt.

Usage:
    python _vc_score_api_vanilla_c.py --input-csv ../Claude_vc_prediction/gt_all_66.csv --outdir ../results/vc_api_63_vanilla_c --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_63_vanilla_c'

# ── Vanilla V0+C — no role framing ───────────────────────────────────────────
...
def main():
    parser = argparse.ArgumentParser(description='Vanilla V0+C VC Scoring (no role framing)')
    ...
    print('=== Vanilla V0+C (no role framing) ===')
    print(f'Model:        {MODEL}')
