"""
Vanilla Zero-Shot VC Scoring — No Role Framing
===============================================
Identical to V0 except the expert-role sentence
"You are a visual complexity (VC) scoring expert for data visualizations."
is removed from the system prompt.

This gives the absolute baseline: just the task definition, no persona.

Usage:
    python _vc_score_api_vanilla.py --input-csv ../Claude_vc_prediction/gt_all_66.csv --outdir ../results/vc_api_63_vanilla --concurrency 5 --model claude-opus-4-6
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_63_vanilla'

# ── Vanilla System Prompt — No role framing ──────────────────────────────────
# Deliberately omits: "You are a visual complexity (VC) scoring expert..."
...
# ── Helpers (identical to _vc_score_api_v0.py) ──────────────────────────────
...
def main():
    parser = argparse.ArgumentParser(description='Vanilla Zero-Shot VC Scoring (no role framing)')
    parser.add_argument('--limit', type=int, default=None)
    parser.add_argument('--overwrite', action='store_true')
    parser.add_argument('--outdir', type=str, default=None)
    parser.add_argument('--input-csv', type=str, default=None,
                        help='CSV with imageName column (e.g. gt_all_66.csv)')
    ...
    print(f'=== Vanilla Zero-Shot Baseline (no role framing) ===')
    print(f'Model:        {MODEL}')
    print(f'Prompt:       Definition only, no expert-role sentence')
