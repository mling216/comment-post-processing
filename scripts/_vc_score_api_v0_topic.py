"""
Claude API Visual Complexity (VC) Scoring — V0+Topic
====================================================
V0 baseline + the 7 topic descriptions (copied from V1). Model is asked to
consider these topics when scoring, but output schema remains V0-style
(single vc_score + explanation). No anchors, no calibration guidance.

Usage:
    python _vc_score_api_v0_topic.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_topic
    python _vc_score_api_v0_topic.py --input-csv ../Claude_vc_prediction/gt_all_46.csv --outdir ../results/vc_api_46gt_v0_topic --concurrency 5
"""
...
DEFAULT_OUTDIR = Path(__file__).parent.parent / 'results' / 'vc_api_46gt_v0_topic'

# ── V0+Topic System Prompt ─ definition + 7 topics, no anchors, no calibration ─
...
def main():
    parser = argparse.ArgumentParser(description='V0+Topic VC Scoring via Claude API')
    ...
    print(f'=== V0+Topic (definition + 7 topics, no anchors) ===')
    print(f'Model:          {MODEL}')
    print(f'Prompt:         V0 + 7 topic descriptions (no calibration, no anchors)')
